...
- 2026-09-06, W4 (augment): the greedy loop is bounded — with `--runs N` it adds exactly N rows, and without `--runs` it stops after at most `n_params` additions — because the spec's stop rule ("every target estimable and either --runs rows added or --runs not given") does not terminate when no candidate can make a target estimable; the bound is the largest possible number of rank increases, so it never cuts a reachable solution short. Exit 3 still reports the unresolved targets.
- 2026-09-06, W4 (augment): a `--for TERM` that the design's formulas do not declare is added to the working (union) model instead of being refused, so an alias can be resolved for an interaction discovered after construct time; the printed model names the addition and the sidecar's `models` is left unchanged.
